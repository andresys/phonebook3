...
(function ($) {
        ...
        html += '<div class="md-toolbar">';
            html += '<div class="btn-toolbar">';
                ...
                html += '<div class="btn-group mr-2">';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnHeader1 + '" class="btn-sm btn btn-outline-secondary" data-btn="h1">H1</button>';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnHeader2 + '" class="btn-sm btn btn-outline-secondary" data-btn="h2">H2</button>';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnHeader3 + '" class="btn-sm btn btn-outline-secondary" data-btn="h3">H3</button>';
                html += '</div>'; // .btn-group

                html += '<div class="btn-group mr-2">';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnBold + '" class="btn-sm btn btn-outline-secondary" data-btn="bold"><span class="fas fa-bold"></span></button>';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnItalic + '" class="btn-sm btn btn-outline-secondary" data-btn="italic"><span class="fas fa-italic"></span></button>';
                html += '</div>'; // .btn-group

                html += '<div class="btn-group mr-2">';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnList + '" class="btn-sm btn btn-outline-secondary" data-btn="ul"><span class="fas fa-list"></span></button>';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnOrderedList + '" class="btn-sm btn btn-outline-secondary" data-btn="ol"><span class="fas fa-list-ol"></span></button>';
                html += '</div>'; // .btn-group

                html += '<div class="btn-group mr-auto">';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnLink + '" class="btn-sm btn btn-outline-secondary" data-btn="link"><span class="fas fa-link"></span></button>';
                    html += '<button type="button" data-mdtooltip="tooltip" title="' + options.label.btnImage + '" class="btn-sm btn btn-outline-secondary" data-btn="image"><span class="fas fa-image"></span></button>';
                    if (options.imageUpload === true) {
                        html += '<div data-mdtooltip="tooltip" title="' + options.label.btnUpload + '" class="btn btn-outline-secondary btn-sm-file"><span class="fas fa-upload"></span><input class="md-input-upload" type="file" multiple accept=".jpg,.jpeg,.png,.gif"></div>';
                    }
                html += '</div>'; // .btn-group

                if (options.preview === true) {
                    html += '<div class="btn-group">';
                        html += '<button type="button" class="btn-sm btn btn-outline-secondary btn-edit active" data-btn="edit"><span class="fas fa-edit"></span> ' + options.label.btnEdit + '</button>';
                        html += '<button type="button" class="btn-sm btn btn-outline-secondary btn-preview" data-btn="preview"><span class="fas fa-eye"></span> ' + options.label.btnPreview + '</button>';
                    html += '</div>'; // .btn-group
                }

                if (options.fullscreen === true) {
                    html += '<div class="btn-group ml-2">';
                        html += '<button type="button" class="btn-sm btn btn-outline-secondary" data-btn="fullscreen"><span class="fas fa-expand-arrows-alt"></span> ' + options.label.btnFullscreen + '</button>';
                    html += '</div>'; // .btn-group
                }
    ...
    var methods = {
        init: function (options) {
            ...
            // Toolbar events
            container.find('.btn-sm').click(function () {
    ...
    $.fn.markdownEditor.defaults = {
        ...
        label: {
            btnHeader1: 'Заголовок 1',
            btnHeader2: 'Заголовок 2',
            btnHeader3: 'Заголовок 3',
            btnBold: 'Жирный',
            btnItalic: 'Курсив',
            btnList: 'Список',
            btnOrderedList: 'Нумерованный список',
            btnLink: 'Гиперссылка',
            btnImage: 'Вставить изображение',
            btnUpload: 'Загрузить изображение',
            btnEdit: 'Править',
            btnPreview: 'Смотреть',
            btnFullscreen: 'Развернуть',
            loading: 'Загрузка'
        }
    };
