...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential curl git pkg-config libyaml-dev imagemagick libvips && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
...
EXPOSE 80
CMD ["./bin/bundle", "exec", "thrust", "./bin/rails", "server"]